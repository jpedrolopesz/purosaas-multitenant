...
@section('account.content')
    ...
    <form enctype="multipart/form-data" action="{{route('central.account.profile.store')}}" method="POST">
        @csrf
            ...
            <div class="px-4 py-5 bg-white space-y-6 sm:p-6 ">
                ...
                <div>
                    ...
                    <div class="mt-1 flex items-center">
                             <span class="inline-block h-10 w-10 rounded-full overflow-hidden bg-gray-100">
                                 <img class="h-full w-full rounded-full" src="/uploads/avatars/{{Auth::guard('admin')->user()->avatar}}"width="38" height="38" alt="User" />
                             </span>
                ...
                <div class="form__input {{$errors->has('email') ? 'has-error' : ''}}">
                    ...
                    <div class="mt-1 rounded-md shadow-sm">
                        <input id="email" type="text" name="email" placeholder="Email Address" value="{{old('email', Auth::guard('admin')->user()->email)}}" required
                    ...
                    <div class="form__input {{$errors->has('name') ? 'has-error' : ''}}" >
                        ...
                        <div class="mt-1 rounded-md shadow-sm " >
                            <input id="name" type="text" name="name" placeholder="Name" value="{{old('name', Auth::guard('admin')->user()->name)}}" required
                                   class="shadow-sm w-full focus:ring-indigo-500 focus:border-indigo-500 mt-1 block sm:text-sm border border-gray-100 rounded-md dark:bg-dark-eval-1"  >
                        </div>
                    </div>
                </div>
