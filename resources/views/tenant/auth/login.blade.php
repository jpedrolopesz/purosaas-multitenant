...
<div class="min-h-full flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
        ...
        <form class="mt-8 space-y-6" action="{{ route('tenant.auth.login') }}" method="POST">
            ...
            <div class="-space-y-px">
                ...
                <div class="mt-6">
                    ...
                    <div class="mt-1 flex rounded-md shadow-sm">
                        <input placeholder="Password" id="password" type="password" name="password" required autocomplete="new-password" value="{{ old('password', '') }}"
                               class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md placeholder-gray-400 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5"/>
                    </div>
                    @error('password')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
        ...
        @if(tenant()->email === 'tenant@demo.com')
            <section>
                <div >
                    ...
                    <div class="hover:border-gray-500 hover:border-solid hover:bg-white hover:text-gray-500 group w-full flex flex-col items-center justify-center rounded-md border-2 border-dashed border-slate-300 text-sm leading-6 text-slate-900 font-medium py-3">
                        <div class="flex">
                            <span>E-mail: </span><span class="underline ml-2">tenant@demo.com</span>
                        </div>
                        <div class="flex">
