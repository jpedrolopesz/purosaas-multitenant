...
@section('account.content')
    ...
    <div class="mt-5 md:mt-0 md:col-span-2 ">
        <form enctype="multipart/form-data" action="{{route('tenant.account.company.store')}}" method="POST">
            @csrf
                ...
                <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex items-center">
                             <span class="inline-block h-10 w-10 rounded-full overflow-hidden bg-gray-100">
                                 @if(tenant()->logo)
                                     <img class="h-full w-full rounded-full" src="/uploads/avatars/{{tenant()->logo}}" width="38" height="38" alt="logo" />
                    ...
                    <div class="form__input {{$errors->has('email') ? 'has-error' : ''}}">
                        <label for="email" class="block text-sm dark:text-white font-medium leading-5 text-gray-700">Email Company</label>
                        <div class="mt-1 rounded-md shadow-sm">
                            <input id="email" type="text" name="email" placeholder="Email Address" value="{{old('email', tenant()->email)}}" required
                        ...
                        <div class="form__input {{$errors->has('company') ? 'has-error' : ''}}" >
                            ...
                            <div class="mt-1 rounded-md shadow-sm " >
                                <input id="company" type="text" name="company" placeholder="Company" value="{{old('company', tenant()->company)}}" required
                                       class="shadow-sm w-full focus:ring-indigo-500 focus:border-indigo-500 mt-1 block sm:text-sm border border-gray-100 rounded-md dark:bg-dark-eval-1"  >
                            </div>
                        </div>
                    </div>
